...
# This script generate the results for beta values less than 1
...
for (i in 1:N_P1) {
  for (j in 1:N_P2) {
    ...
    CRM <- function(t, state, parameter) {
      ...
      delta_f_A_V1= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*(((  (parameter["f"]) * state["R"]  )    - parameter["P1"]  - parameter["P2"] )-(((  (parameter["f"]-parameter["C_V1"]) * state["R"]    - parameter["CR"]))    - parameter["PR"]*parameter["P1"]  - 0.5*parameter["P2"])))))
      
      delta_f_A_V2= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*(((  (parameter["f"]) * state["R"]  )    - parameter["P1"]  - parameter["P2"])-(((  (parameter["f"]-parameter["C_V2"]) * state["R"]    - parameter["CR"]))   - parameter["PR"]*parameter["P2"] - 0.5*parameter["P1"])))))
      
      delta_f_V1_V12= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*((((  (parameter["f"]-parameter["C_V1"]) * state["R"]    - parameter["CR"]))   - parameter["PR"]*parameter["P1"]   - 0.5*parameter["P2"] )-(((  (parameter["f"]-parameter["C_V12"]) * state["R"]    - 2 * parameter["CR"]- parameter["PR"]*parameter["P1"]- parameter["PR"]*parameter["P2"]))   )))))
      
      delta_f_V2_V12= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*((((  (parameter["f"]-parameter["C_V2"]) * state["R"]    - parameter["CR"]))  - parameter["PR"]*parameter["P2"]   - 0.5*parameter["P1"] )-(((  (parameter["f"]-parameter["C_V12"]) * state["R"]    - 2 * parameter["CR"]- parameter["PR"]*parameter["P1"]- parameter["PR"]*parameter["P2"]))   )))))
      
	  
	  delta_f_V1_A= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*(-((  (parameter["f"]) * state["R"]  )    - parameter["P1"]  - parameter["P2"] )+(((  (parameter["f"]-parameter["C_V1"]) * state["R"]    - parameter["CR"]))    - parameter["PR"]*parameter["P1"]  - 0.5*parameter["P2"])))))
      
      delta_f_V2_A= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*(-((  (parameter["f"]) * state["R"]  )    - parameter["P1"]  - parameter["P2"])+(((  (parameter["f"]-parameter["C_V2"]) * state["R"]    - parameter["CR"]))   - parameter["PR"]*parameter["P2"] - 0.5*parameter["P1"])))))
      
      delta_f_V12_V1= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*(-(((  (parameter["f"]-parameter["C_V1"]) * state["R"]    - parameter["CR"]))   - parameter["PR"]*parameter["P1"]   - 0.5*parameter["P2"] )+(((  (parameter["f"]-parameter["C_V12"]) * state["R"]    - 2 * parameter["CR"]- parameter["PR"]*parameter["P1"]- parameter["PR"]*parameter["P2"]))   )))))
      
      delta_f_V12_V2= parameter["chi_max"] *  (1 / (1 +exp(parameter["teta"]*(-(((  (parameter["f"]-parameter["C_V2"]) * state["R"]    - parameter["CR"]))  - parameter["PR"]*parameter["P2"]   - 0.5*parameter["P1"] )+(((  (parameter["f"]-parameter["C_V12"]) * state["R"]    - 2 * parameter["CR"]- parameter["PR"]*parameter["P1"]- parameter["PR"]*parameter["P2"]))   )))))
     
	  
      # rates of change
      
      dA <-
        (  (parameter["f"]) * state["R"]  *state["A"])    - parameter["P1"] * state["A"] - parameter["P2"] * state["A"] - delta_f_A_V1* state["A"]-delta_f_A_V2* state["A"] + delta_f_V1_A* state["V1"] + delta_f_V2_A* state["V2"]
      dV1 <-
        ((  (parameter["f"]-parameter["C_V1"]) * state["R"]    - parameter["CR"])*state["V1"])    + delta_f_A_V1* state["A"] - delta_f_V1_A* state["V1"] - 0.5*parameter["P2"] * state["V1"]- parameter["PR"]*parameter["P1"] * state["V1"] - delta_f_V1_V12* state["V1"]+ delta_f_V12_V1* state["V12"]
      dV2 <-
        ((  (parameter["f"]-parameter["C_V2"]) * state["R"]    - parameter["CR"])*state["V2"])     + delta_f_A_V2* state["A"] - delta_f_V2_A*state["V2"] - 0.5*parameter["P1"] * state["V2"]- parameter["PR"]*parameter["P2"] * state["V2"] - delta_f_V2_V12* state["V2"] + delta_f_V12_V2* state["V12"]
      dV12 <-
        ((  (parameter["f"]-parameter["C_V12"]) * state["R"]    - 2 * parameter["CR"])*state["V12"])  - parameter["PR"]*parameter["P1"] * state["V12"]- parameter["PR"]*parameter["P2"] * state["V12"]   + delta_f_V1_V12* state["V1"] + delta_f_V2_V12* state["V2"] - delta_f_V12_V1* state["V12"] - delta_f_V12_V2* state["V12"]
      dP <-
        parameter["r"] * (1 - state["R"] / parameter["KR"]) * state["R"] -   (parameter["f"]) *state["R"] * state["A"]   -   (parameter["f"]-parameter["C_V1"]) *state["R"] * state["V1"]    -   (parameter["f"]-parameter["C_V2"]) *state["R"] * state["V2"]    -   (parameter["f"]-parameter["C_V12"]) *state["R"] * state["V12"]   
      dPP <-
        parameter["r"] * (1 - state["R_asterisc"] / parameter["KR"]) * state["R_asterisc"] -   (parameter["f"]) *state["R_asterisc"] * state["A"]   -   (parameter["f"]) *state["R_asterisc"] * state["V1"]   -   (parameter["f"]) *state["R_asterisc"] * state["V2"]   -   (parameter["f"]) *state["R_asterisc"] * state["V12"]  
